...
metadat <- fread("catchment_list.csv")[catchment == "ybbs"]
shareDir <- file.path("/Volumes/Data/catchments", metadat$catchment, metadat$version)
...
# get discharge
qSites = data.table(dbReadTable(metabDB, "siteDischargeView"))[expedition == 'YbbsFall2019']
...
for(x in names(geom)) {
	writeRaster(geom[[x]], file.path(gdir, paste0(x, "_", "YbbsFall2019.tif")), 
...
metadat <- fread("catchment_list.csv")[catchment == "kamp"]
shareDir <- file.path("/Volumes/Data/catchments", metadat$catchment, metadat$version)
...
# get discharge
qSites = data.table(dbReadTable(metabDB, "siteDischargeView"))[expedition == 'KampFall2019']
qSites = st_as_sf(qSites, coords = c('x', 'y'), crs = qSites$epsg[1])
qSites$catchmentArea <- raster::extract(catchmentAreas, qSites)
...
for(x in names(geom)) {
	writeRaster(geom[[x]], file.path(gdir, paste0(x, "_", "KampFall2019.tif")), 
			options = c("COMPRESS=LZW", "PREDICTOR=3"))
}
